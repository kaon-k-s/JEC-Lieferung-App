...
import { useNavigation, useRoute } from '@react-navigation/native'; // Importiere Hooks für Navigation und Route
import styles from './styles';  // Importiere die Styles

const ItemListPage = () => {
  const [items, setItems] = useState([]);
  const [cartItems, setCartItems] = useState([]); // Zustand zum Speichern der Warenkorb-Artikel
  const route = useRoute();
  const { category, categoryType } = route.params;
  const navigation = useNavigation(); // Für die Navigation

  useEffect(() => {
    // Hole Artikel basierend auf der Kategorie und dem Typ
    ...
    axios.get(apiUrl)
      .then(response => {
        setItems(response.data);  // Setze die Artikel-Daten aus der Antwort
      })
      .catch(error => {
        console.error('Fehler beim Abrufen der Daten', error);  // Behandle Fehler
      });
  }, [category, categoryType]);

  // Funktion, um einen Artikel in den Warenkorb hinzuzufügen
  const addToCart = (item) => {
    setCartItems(prevItems => [...prevItems, item]);
  };

  // Navigation zur Warenkorb-Seite (Warenkorb) und übergebe die Warenkorb-Artikel
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Button zur Navigation in den Warenkorb */}
